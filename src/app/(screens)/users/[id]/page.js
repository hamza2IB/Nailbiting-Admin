...
function UserDetail({ params }) {
  ...
  const fetchUser = async () => {
    if (userId) {
      try {
        ...
        setUser(userData);
        const nailPhotos = userData.selfAssessmentResults
          .map((result) => result.nailPhotos)
          .flat();
  ...
  const filterEvaluations = (evaluations) => {
    ...
    const filtered = evaluations.filter((evaluation) => {
      ...
      return (
        (!selectedMonth || selectedMonth === evaluationMonth) &&
        (!selectedYear || selectedYear === evaluationYear)
      );
    });
        ...
        <>
          <div className="flex flex-row items-center justify-end mb-5">
            <div
              ...
              className="p-3 gap-2 flex flex-row cursor-pointer relative border rounded-lg"
            >
              <svg
                width="20"
                height="20"
                  ...
                  {months.map((month) => (
                    <div
                      ...
                      className={`p-2 cursor-pointer items-center justify-center rounded-md hover:bg-[#ECFDF3] ${
                        selectedMonth === month
                          ? "bg-[#199A8E] hover:text-black text-white"
                          : ""
                      }`}
          ...
          <div className="overflow-auto max-h-[500px]">
            {filteredEvaluations.length === 0 ? (
              <p className="text-center text-gray-500">評価が見つかりません</p>
            ) : (
                ...
                <tbody>
                  {filteredEvaluations.map((evaluation, index) => (
                    ...
                    >
                      ...
                      <td className="py-4 px-6">
                        {renderStars(evaluation?.responses[1]?.answer || "N/A")}
                      </td>
    ...
    return (
      ...
      >
        <div className="relative" onClick={(e) => e.stopPropagation()}>
          ...
          <button
            onClick={onClose}
            className="absolute -top-8 -right-2 text-white p-4 text-[50px]"
          >
            &times;
          </button>
        </div>
  ...
  const renderTabContent = () =>
    activeTab === "自己評価" ? renderEvaluationsTable() : renderPhotosGrid();
      ...
      <div className="pt-5">
        <div className="mx-auto px-4 py-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <span className="text-[30px] text-[30px] text-[#000000]">
              ユーザー詳細
            </span>
          </button>
        </div>
