          ...
          <Formik
            ...
            onSubmit={async (values, { resetForm }) => {
              try {
                ...
                let response;
                if (isEditMode) {
                  response = await updateNews(newsId, values) // Update existing news
                } else {
                  response = await addNews(values) // Add new news
                }
                ...
                // If the error is related to push notifications but the news was created
                if (error.response?.status === 400 && 
                    error.response?.data?.message?.includes('push notification')) {
                  // The news was likely created, just the push notification failed
                  resetForm()
                  router.push('/news')
                } else {
                  // Handle other errors
