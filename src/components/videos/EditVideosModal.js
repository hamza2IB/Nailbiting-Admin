...
const validationSchema = Yup.object().shape({
	youtubeLink: Yup.string().url('有効なURLを入力してください').required('リンクは必須です'),
	title: Yup.string().max(50, 'タイトルは50文字以内で入力してください').required('タイトルは必須です'),
	description: Yup.string().max(150, '説明は150文字以内で入力してください').required('説明は必須です'), // Ensure it's required
})

const VideosModal = ({ isOpen, toggleModal, formData, handleChange, handleSubmit }) => {
	...
	return (
		<div className='fixed inset-0 p-10 flex items-center justify-center z-50 bg-black bg-opacity-50'>
			<div id='modal-content' className='bg-white  w-full max-w-[1320px]'>
				<h1 className='text-xl font-semibold text-center border-b py-6 '>動画追加</h1>
					...
					<div className='mx-auto '>
						<Formik
							...
							{({ values, errors, touched, setFieldValue }) => {
								const currentLength = values.description.length
								return (
									<Form className='space-y-6 '>
										<RenderField
											...
											error={errors.youtubeLink || (touched.youtubeLink && errors.youtubeLink)}
										/>
										<RenderField
											id='title'
											label='動画タイトル'
